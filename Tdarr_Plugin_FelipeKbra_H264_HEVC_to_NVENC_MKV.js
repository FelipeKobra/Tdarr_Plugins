...
const details = () => ({
  ...
  Name: 'FelipeKbra - H264/HEVC to NVENC Focused on MKV Containers',
  Type: 'Video',
  Operation: 'Transcode',
  Description:
...
function buildVideoConfiguration(inputs, file, logger) {
  ...
  function videoProcess(stream, id) {
    ...
    if (stream.codec_name === 'png') {
      ...
      // Add output video encoding settings (Updated to mirror FelipeKbra's parameters while preserving original metadata tracking)
      configuration.AddOutputSetting(`-c:v hevc_nvenc -tag:v hvc1 -profile:v main10 -pix_fmt:v p010le -gpu 0 -surfaces 64 -qmin 0 -cq:v ${cq} -b:v ${bitrateTarget}k -maxrate:v ${bitrateMax}k -preset slow -multipass fullres -rc-lookahead 32 -spatial_aq:v 1 -aq-strength:v 15 -threads 1 -metadata:s:v:0 COPYRIGHT=processed`);
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  // Start with input-specific options (Added -hwaccel cuda to match FelipeKbra's implementation)
